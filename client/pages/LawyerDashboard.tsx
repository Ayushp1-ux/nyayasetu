...
import { MessageCircle, FileText, Star, ChevronDown, ChevronUp } from "lucide-react";
...
export default function LawyerDashboard() {
  ...
  const [myRating, setMyRating] = useState({ avg: 0, count: 0, reviews: [] as any[] });
  ...
  useEffect(() => {
    async function fetchBookings() {
      ...
      const { data, error } = await supabase
        .from("bookings")
        .select("id, user_id, booking_time, status, created_at, updated_at, user_profile:profiles!bookings_user_id_fkey(full_name)")
        .eq("lawyer_id", user.id)
        .order("created_at", { ascending: false });
      if (!error && data) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4">
      <div className="max-w-7xl mx-auto mb-8">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h2 className="text-2xl font-bold">Your Rating: ⭐ {myRating.avg}</h2>
                  <p className="text-sm opacity-90">{myRating.count} total review{myRating.count > 1 ? 's' : ''} • Click to {showRatingDetails ? 'hide' : 'view'} details</p>
                </div>
              </div>
              {showRatingDetails ? (
              ...
              <div className="px-6 pb-6 space-y-3 animate-in slide-in-from-top duration-200">
                <div className="pt-4 border-t border-yellow-400">
                  ...
                  {myRating.reviews.map((review: any, idx: number) => (
                    <div key={idx} className="bg-yellow-400/20 backdrop-blur-sm p-4 rounded-lg mb-3">
                      <div className="flex gap-1 mb-2">
                        {[...Array(5)].map((_, i) => (
                          <Star 
                            key={i} 
                            className={`h-4 w-4 ${i < review.rating ? 'fill-white text-white' : 'text-white/40'}`}
                          />
                        ))}
                      </div>
                      {review.review && (
                        <p className="text-sm leading-relaxed mb-2">"{review.review}"</p>
                      )}
        ...
        {/* BOOKINGS SECTION */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-primary mb-3">Consultation Bookings</h2>
          {loadingBookings ? (
            ...
            <div className="space-y-4">
              {bookings.map((booking) => {
                const formattedBookingTime = new Date(booking.booking_time).toLocaleString('en-IN', {
                ...
                >
                  <div>
                    <div className="font-semibold">
                      {booking.user_profile && booking.user_profile.length > 0 && booking.user_profile[0]?.full_name
                        ? booking.user_profile[0].full_name
                        : "Unknown User"}
                    </div>
                    <div className="text-sm mt-1">
                      Date & Time: <span className="font-mono">{formattedBookingTime}</span>
                    </div>
                  ...
                  <div className="flex flex-col gap-2 mt-2 md:mt-0 min-w-[170px]">
                    ...
                    {booking.status === "pending" ? (
                      ...
                      <span className="text-xs text-gray-400 mt-2">
                        Last updated: {new Date(booking.updated_at).toLocaleString('en-IN', {
                          timeZone: 'Asia/Kolkata',
                          dateStyle: 'medium',
                          timeStyle: 'short'
                        })}
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">View and respond to client chats</p>
              </div>
            </div>
          </Link>
          <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-6 rounded-lg shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-green-100 text-sm">{questions.length} total questions</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-1/3 bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
            ...
            <h3 className="font-bold text-lg text-gray-900 dark:text-white mb-4">User Questions</h3>
            {questions.length === 0 && (
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto">
              {questions.map((q) => (
                ...
                >
                  <div className="font-semibold text-gray-900 dark:text-white mb-1 line-clamp-1">
                    {q.title}
                  </div>
